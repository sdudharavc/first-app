angular.module('widgetsetting', []).controller('WidgetSettingsCtrl',function($scope, GsmServices, $modalInstance, widget,
						$rootScope, UtilsFactory, index, Messages,
						Notification, Constants, $timeout, $filter, $interval) {
					...
					$scope.datePickerChange = function(val) {
						var start = moment().subtract(parseInt(val), 'day')
								.format(timeFormat);
					...
					$(function() {
						...
						if (UtilsFactory.getDateRange(widget.childWidgets) !== undefined
								&& UtilsFactory
										.getDateRange(widget.childWidgets) !== null
								&& UtilsFactory
										.getDateRange(widget.childWidgets).length > 0) {
							start = UtilsFactory
									.getDateRange(widget.childWidgets)[0];
							end = UtilsFactory
									.getDateRange(widget.childWidgets)[1];
						}
					...
					$scope.addNewChoice = function() {
						if ($scope.choices.length < 5) {
							...
							if (widget.jsfunctionname === "pue_line_chart"
									|| widget.jsfunctionname === "pue_area_chart"
									|| widget.jsfunctionname === "pue_bar_chart") {
								var pueData = JSON
										.parse(UtilsFactory
												.getHistoryGraphData(widget.childWidgets));
								$scope.choices
										.push({
											'id' : newItemNo,
											'form' : {
												'formula' : pueData.widgetSetting.formula
											}
										});
							} else if (widget.jsfunctionname === 'bar_thresold'
									|| widget.jsfunctionname === 'line_thresold'
									|| widget.jsfunctionname === 'area_thresold') {
								var thresoldData = JSON
										.parse(UtilsFactory
												.getHistoryGraphData(widget.childWidgets));
								$scope.choices
										.push({
											...
											'form' : {
												'checkFilter' : false,
												'datacenter' : thresoldData.widgetSetting.datacenter,
												'cabinet' : [],
												'floor' : [],
							...
							} else if (widget.jsfunctionname == "line_sensor"
									|| widget.jsfunctionname == "area_sensor"
									|| widget.jsfunctionname == "bar_sensor") {
								var sensorData = JSON
										.parse(UtilsFactory
												.getHistoryGraphData(widget.childWidgets));
								$scope.choices
										.push({
											'id' : newItemNo,
											'form' : {
												'datacenter' : sensorData.widgetSetting.datacenter,
												'floor' : [],
												'cabinet' : [],
												'sensor' : $scope.form.sensorList
											}
										});
							} else if (widget.jsfunctionname === "line_getwithoutpuehistory"
									|| widget.jsfunctionname === "bar_getwithoutpuehistory"
									|| widget.jsfunctionname === "area_getwithoutpuehistory") {
								var withoutPueHistory = JSON
										.parse(UtilsFactory
												.getHistoryGraphData(widget.childWidgets));
								$scope.choices
										.push({
											'id' : newItemNo,
											'form' : {
												'formula' : withoutPueHistory.widgetSetting.formula
											}
										});
							}
						} else {
							Notification.error({
								message : Messages.FIELD_LIMIT_REACHED,
					...
					$scope.getMetrixValue = function(value) {
						if (value !== undefined && value !== null) {
							var inputParam = {
								"threshold_type" : value.walt_threshold_type
							};
							GsmServices.getMetrix(inputParam).success(
									function(response) {
					...
					$scope.resetThresholdGauge = function(id) {
						if (id) {
							...
							if ($scope.choices[id - 1].form == undefined) {
								$scope.choices[id - 1].form = {};
							}
							$scope.choices[id - 1].form.floor = [];
							$scope.choices[id - 1].form.cabinet = [];
							if ($scope.choices[id - 1]
									&& !$scope.choices[id - 1].checkFilter) {
								$scope.getpowermetricitemForChoice(
										$scope.choices[id - 1], $scope.data);
							}
						} else {
							$scope.data.datacenter = {};
							$scope.data.floor = {};
							$scope.data.cabinet = {};
							if (!$scope.data.checkFilter) {
					...
					$scope.getpowermetricitem = function(value) {
						if (value !== undefined
								&& value.walt_threshold_type !== undefined
								&& value.walt_metrix !== undefined) {
							if ((value.cabinet == undefined || value.cabinet.length == 0)
									&& (value.floor == undefined || value.floor.length == 0)
									&& (value.datacenter == undefined || value.datacenter.length == 0)) {
								var inputParam = {
									"threshold_type" : value.walt_threshold_type,
									"walt_metrix" : value.walt_metrix
								};
								GsmServices
										.getpowermetricitem(inputParam)
										.success(
												function(response) {
													$scope.form.powerMetriceItem = response.responseObject.data;
												});
							} else if (value.cabinet != undefined
									&& value.cabinet.CabinetKey != undefined) {
								$scope.getnodebycabinetForGauge(value);
							} else if (value.floor != undefined
									&& value.floor.FloorKey != undefined) {
								$scope.getnodebyfloorForGauge(value);
							} else if (value.datacenter != undefined
									&& value.datacenter.DCKey != undefined) {
					...
					$scope.getpowermetricitemForChoice = function(value, data) {
						if (value == null
								&& data.walt_threshold_type !== undefined
								&& data.walt_metrix !== undefined) {
							value = {};
							var inputParam = {
								"threshold_type" : data.walt_threshold_type,
								"walt_metrix" : data.walt_metrix
							};
							GsmServices
									.getpowermetricitem(inputParam)
									.success(
												...
												_
														.filter(
																...
																function(
																	...
																	// $scope.choices[i].datacenter
																	// =
																	// undefined;
																	// $scope.choices[i].floor
																	// =
																	// undefined;
																	// $scope.choices[i].cabinet
																	// =
																	// undefined;
																	// $scope.choices[i].powerMetriceItem
																	// =
																	// undefined;
																});
											});
					...
					function getpowermetricitemnewchoice(id) {
						if ($scope.data.walt_threshold_type
								&& $scope.data.walt_metrix) {
							var inputParam = {
								"threshold_type" : $scope.data.walt_threshold_type,
								"walt_metrix" : $scope.data.walt_metrix
							};
							GsmServices
									.getpowermetricitem(inputParam)
									.success(
					...
					$scope.getnodebydc = function(value) {
						if (value !== undefined
								&& value !== null
								&& $scope.data.walt_threshold_type !== undefined
								&& $scope.data.walt_metrix !== undefined) {
							if (value.datacenter) {
								var inputParam = {
									"dcKey" : value.datacenter.DCKey,
									'type' : $scope.data.walt_threshold_type,
									'metrix' : $scope.data.walt_metrix
								};
								GsmServices
										.getnodebydc(inputParam)
										.success(
												function(response) {
													$scope.choices[value.id - 1].form.powerMetriceItem = response.responseObject.data;
												});
							} else if ($scope.data.walt_threshold_type !== undefined
									&& $scope.data.walt_metrix !== undefined) {
								...
								$scope.getpowermetricitemForChoice(null,
										$scope.data)
							}
						}
					}
					...
					$scope.getnodebyfloor = function(value) {
						if (value !== undefined
							...
							if (value.floor) {
								var inputParam = {
									'floorid' : value.floor.FloorKey,
									'type' : $scope.data.walt_threshold_type,
									'metrix' : $scope.data.walt_metrix
								};
								GsmServices
										.getnodebyfloor(inputParam)
										.success(
												function(response) {
													$scope.choices[value.id - 1].form.powerMetriceItem = response.responseObject.data;
												});
					...
					$scope.getnodebycabinet = function(value) {
						if (value !== undefined
							...
							if (value.cabinet) {
								var inputParam = {
									"cabinetKey" : value.cabinet.CabinetKey,
									'type' : $scope.data.walt_threshold_type,
									'metrix' : $scope.data.walt_metrix
								};
								GsmServices
										.getnodebycabinet(inputParam)
										.success(
												function(response) {
													$scope.choices[value.id - 1].form.powerMetriceItem = response.responseObject.data;
												});
					...
					$scope.getnodebydcForSensor = function(value) {
						if (value !== undefined && value !== null
								&& $scope.data.sensortype !== undefined) {
							if (value.datacenter) {
								var inputParam = {
									"dcKey" : value.datacenter.DCKey,
									'type' : 'Envsensor',
									'sensortype' : $scope.data.sensortype
								};
								GsmServices
										.getnodebydc(inputParam)
										.success(
					...
					$scope.getsensorbyfloorusingcabinet = function(value,
							floor, id) {
						if (value !== undefined && value !== null
								&& $scope.form.sensorList !== undefined
								&& $scope.form.sensorList !== null) {
							if (widget.jsfunctionname == "sensor_gauge") {
								$scope.form.sensor = _
										.filter(
												$scope.form.sensorList,
								...
								$scope.choices[id - 1].form = {}
								$scope.choices[id - 1].form.sensor = _
										.filter(
												$scope.form.sensorList,
												function(row) {
													return row.CabinetName == value.CabinetName;
					...
					if (widget.jsfunctionname === "walt_thresold") {
						var thresoldData = JSON.parse(UtilsFactory
								.getWaltThresholdData(widget.childWidgets));
						if (thresoldData.setting.powerMetriceItem !== undefined) {
							...
							if ($scope.data.feed == 'null'
									&& $scope.data.phase == 'null') {
							...
							console.log($scope.form,
									$scope.data.powerMetriceItem);
							if ($scope.data.updateInterval == "Minute"
									|| $scope.data.updateInterval == "Second"
									|| $scope.data.updateInterval == "Hour") {
						...
						var thresoldData = JSON.parse(UtilsFactory
								.getHistoryGraphData(widget.childWidgets));
						if (thresoldData.setting !== null
								&& thresoldData.setting.walt_threshold_type !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								walt_threshold_type : thresoldData.setting.walt_threshold_type,
								walt_metrix : thresoldData.setting.walt_metrix,
								datatype : thresoldData.setting.datatype,
								dateRange : $filter('DateAdderAndFormatter')(
										thresoldData.setting.dateRangeFrom,
										thresoldData.setting.dateRangeTo),
								updateInterval : thresoldData.setting.update_interval.intervalType
							};
							if (thresoldData.setting.days != 'null'
									&& thresoldData.setting.days != undefined) {
								$scope.data.days = thresoldData.setting.days;
							} else {
								$scope.data.days = "";
							}

							if ($scope.data.datatype == "Average"
									|| $scope.data.datatype == "Minimum"
									|| $scope.data.datatype == "Maximum") {
							...
							$scope.form.updateInterval = thresoldData.widgetSetting.timeInterval;
							var dateRange = [
									$filter('DateAdderAndFormatter')(
											thresoldData.setting.dateRangeFrom),
									$filter('DateAdderAndFormatter')(
											thresoldData.setting.dateRangeTo) ];
							UtilsFactory.setDateRange(dateRange,
									widget.childWidgets);
							...
							_
									.each(
											...
											function(choice, i) {
												...
												if ($scope.choices[i].feed == 'null'
														&& $scope.choices[i].phase == 'null') {
												...
												if ($scope.choices[i].floor
														&& $scope.choices[i].floor.length !== 0) {
													$timeout(
															function() {
																$scope
																		.getCabinet(
												...
												if ($scope.choices[i].cabinet
														&& $scope.choices[i].cabinet.length !== 0) {
													$timeout(
															function() {
																$scope
																		.getnodebycabinet($scope.choices[i]);
															}, 10)
													...
													$timeout(
															function() {
																$scope
																		.getnodebyfloor($scope.choices[i]);
															}, 10)
												} else {
													$timeout(
															function() {
																$scope
																		.getnodebydc($scope.choices[i]);
															}, 10)
												}
						...
						var floorData = JSON.parse(UtilsFactory
								.getFloorImage(widget.childWidgets));
						if (floorData.setting !== null
								&& floorData.setting.datacenter !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								datacenter : floorData.setting.datacenter,
								floor : floorData.setting.floor,
								updateInterval : floorData.setting.update_interval.intervalType
							};
						...
						var spiderdata = JSON.parse(UtilsFactory
								.getSpiderchartData(widget.childWidgets));
						if (spiderdata.setting !== null
								&& spiderdata.setting.cabinet !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								datacenter : spiderdata.setting.datacenter,
								floor : spiderdata.setting.floorlist,
								updateInterval : spiderdata.setting.update_interval.intervalType
							};
							if ($scope.data.updateInterval == "Minute"
									|| $scope.data.updateInterval == "Second"
									|| $scope.data.updateInterval == "Hour") {
							...
							_.each(spiderdata.setting.cabinet, function(choice,
									i) {
								$scope.choices[i] = {};
								$scope.choices[i].id = i + 1;
								$scope.choices[i].form = {};
						...
						var spiderFloordata = JSON.parse(UtilsFactory
								.getSpiderchartData(widget.childWidgets));
						if (spiderFloordata.setting !== null
								&& spiderFloordata.setting.floor !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								datacenter : spiderFloordata.setting.datacenter,
								updateInterval : spiderFloordata.setting.update_interval.intervalType
							};
							if ($scope.data.updateInterval == "Minute"
									|| $scope.data.updateInterval == "Second"
									|| $scope.data.updateInterval == "Hour") {
							...
							_.each(spiderFloordata.setting.floor, function(
									choice, i) {
								$scope.choices[i] = {};
								$scope.choices[i].id = i + 1;
								$scope.choices[i].form = {};
					...
					} else if (widget.jsfunctionname === "pue_line_chart"
							|| widget.jsfunctionname === "pue_area_chart"
							|| widget.jsfunctionname === "pue_bar_chart") {
						var pueData = JSON.parse(UtilsFactory
								.getHistoryGraphData(widget.childWidgets));
						if (pueData.setting !== null
								&& pueData.setting.formula !== "null"
								&& pueData.setting.formula !== undefined) {
							$scope.initializeForm();
							_
									.each(
							...
							$scope.data = {
								dateRange : $filter('DateAdderAndFormatter')(
										pueData.setting.dateRangeFrom,
										pueData.setting.dateRangeTo),
								datatype : pueData.setting.datatype,
								updateInterval : pueData.setting.update_interval.intervalType,
							};
							if (pueData.setting.days != 'null'
									&& pueData.setting.days != undefined) {
							...
							|| widget.jsfunctionname === "bar_getwithoutpuehistory"
							|| widget.jsfunctionname === "area_getwithoutpuehistory") {
						var withoutPueHistory = JSON.parse(UtilsFactory
								.getHistoryGraphData(widget.childWidgets));
						if (withoutPueHistory.setting !== null
								&& withoutPueHistory.setting.formula !== "null"
								&& withoutPueHistory.setting.formula !== undefined) {
							$scope.initializeForm();
							_
									.each(
							...
							$scope.data = {
								dateRange : $filter('DateAdderAndFormatter')
										(
												withoutPueHistory.setting.dateRangeFrom,
												withoutPueHistory.setting.dateRangeTo),
								datatype : withoutPueHistory.setting.datatype,
								updateInterval : withoutPueHistory.setting.update_interval.intervalType
							};
							...
							if (withoutPueHistory.setting.days != 'null'
									&& withoutPueHistory.setting.days != undefined) {
							...
							|| widget.jsfunctionname == "area_sensor"
							|| widget.jsfunctionname == "bar_sensor") {
						var sensorData = JSON.parse(UtilsFactory
								.getHistoryGraphData(widget.childWidgets));
						if (sensorData.setting !== null
								&& sensorData.setting.choices !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								dateRange : $filter('DateAdderAndFormatter')(
										sensorData.setting.dateRangeFrom,
										sensorData.setting.dateRangeTo),
								sensortype : sensorData.setting.sensortype,
								datatype : sensorData.setting.datatype,
								metrixtype : sensorData.setting.metrixtype,
								updateInterval : sensorData.setting.update_interval.intervalType
							};
							...
							if (sensorData.setting.days != 'null'
									&& sensorData.setting.days != undefined) {
											...
											function(choice, i) {
												...
												if ($scope.choices[i].cabinet) {
													$timeout(
															function() {
																$scope
																		.getnodebycabinetForSensor($scope.choices[i]);
															}, 10)
												} else if ($scope.choices[i].floor) {
													$timeout(
															function() {
																$scope
																		.getnodebyfloorForSensor($scope.choices[i]);
															}, 10)
												} else {
													$timeout(
															function() {
																$scope
																		.getnodebydcForSensor($scope.choices[i]);
															}, 10)
												}
											})
						} else {
						...
						var SensorGaugeData = JSON.parse(UtilsFactory
								.getSensorGaugeData(widget.childWidgets));
						if (SensorGaugeData.setting.Sensor !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								datacenter : SensorGaugeData.setting.datacenter,
								floor : SensorGaugeData.setting.floor,
								cabinet : SensorGaugeData.setting.cabinet,
								sensortype : SensorGaugeData.setting.sensortype,
								metrixtype : SensorGaugeData.setting.metrixtype,
								sensor : SensorGaugeData.setting.Sensor,
								updateInterval : SensorGaugeData.setting.update_interval.intervalType,
								headerColorInput : SensorGaugeData.setting.headerColorInput,
								dayformax : SensorGaugeData.setting.dayformax
							};
							if ($scope.data.dayformax == 'NULL') {
								$scope.data.dayformax = null;
							}
							...
							if ($scope.data.cabinet) {
								$timeout(
										function() {
											$scope
													.getnodebycabinetForGaugeForSensor($scope.data);
										}, 10)
							} else if ($scope.data.floor) {
								$timeout(
										function() {
											$scope
													.getnodebyfloorForGaugeForSensor($scope.data);
										}, 10)
							} else {
								$timeout(
										function() {
											$scope
													.getnodebydcForGaugeForSensor($scope.data);
										}, 10)
							}
						...
						var rackElevationData = JSON.parse(UtilsFactory
								.getRackElevationChartData());
						if (rackElevationData.setting !== null
								&& rackElevationData.setting !== undefined
								&& rackElevationData.setting.datacenter !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								datacenter : rackElevationData.setting.datacenter,
								floor : rackElevationData.setting.floorlist,
								cabinet : rackElevationData.setting.cabinet,
								updateInterval : rackElevationData.setting.update_interval.intervalType
							};
						...
						var infoData = JSON.parse(UtilsFactory
								.getInfoChartData(widget.childWidgets));
						if (infoData.setting != null
								&& infoData.setting.datacenter !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								datacenter : infoData.setting.datacenter,
								floor : infoData.setting.floor,
								cabinet : infoData.setting.cabinet,
								updateInterval : infoData.setting.update_interval.intervalType
							};
					...
					} else if (widget.jsfunctionname === "pue_live"
							|| widget.jsfunctionname === "gauge_getwithoutpuehistory") {
						var pueData = JSON.parse(UtilsFactory
								.getPueData(widget.childWidgets));
						if (pueData.setting !== null
								&& pueData.setting.formula !== undefined) {
							$scope.initializeForm();
							$scope.data = {
								formula : pueData.setting.formula,
								updateInterval : pueData.setting.update_interval.intervalType,
								headerColorInput : pueData.setting.headerColorInput,
								dayformax : pueData.setting.dayformax
							};
							if ($scope.data.dayformax == 'NULL') {
								$scope.data.dayformax = null;
							}
					...
					$scope.setSaveSettings = function() {
						...
						inputdata.widgetsData = {
							'sizeX' : $scope.form.sizeX,
							'sizeY' : $scope.form.sizeY,
							'col' : $scope.form.col,
							'row' : $scope.form.row,
							'id' : ($scope.form.jsfunctionname == 'getRackElevationData') ? $rootScope.rackChildId
									: widget.childWidgets,
							'name' : $scope.form.name,
							'widgetname' : $scope.form.widgetname,
						...
						if (!form.$invalid) {
							...
							if (widget.jsfunctionname === "getFloor") {
								...
								inputdata.settings = {
									...
									'floor' : data.floor,
									'update_interval' : {
										'intervalType' : data.updateInterval,
										'intervalTime' : $scope.form.time
									}
								};
								inputdata.dashboardId = $rootScope.selectedDashboardIdForAddWidget;

								GsmServices
										.save_widget_setting(inputdata)
										.success(
												function(response) {
													$scope.dismiss();
													$rootScope
															.$emit(
																	"getWidgetDataAfterSave",
																	inputdata.widgetsData,
																	index);
													if ($scope.intervals) {
														$interval
																.cancel($scope.intervals[widget.childWidgets])
								...
								inputdata.settings = {
									...
									'walt_threshold_type' : data.walt_threshold_type,
									'walt_metrix' : data.walt_metrix,
									'feed' : data.feed,
									'phase' : data.phase,
									'checkFilter' : data.checkFilter,
									'powerMetriceItem' : data.powerMetriceItem,
									'dayformax' : data.dayformax,
									'update_interval' : {
										'intervalType' : data.updateInterval,
										'intervalTime' : $scope.form.time
									},
									'headerColorInput' : typeof data.headerColorInput == undefined ? 'false'
											: data.headerColorInput
								};
								inputdata.dashboardId = $rootScope.selectedDashboardIdForAddWidget;
								...
								inputdata.settings = {
									...
									'days' : data.days,
									'dateRangeFrom' : $filter(
											'DateDividerAndFormatter')(
											data.dateRange)[0],
									'dateRangeTo' : $filter(
											'DateDividerAndFormatter')(
											data.dateRange)[1],
									'datatype' : data.datatype,
									'subdatatype' : data.subdatatype,
									'update_interval' : {
										'intervalType' : data.updateInterval,
										'intervalTime' : $scope.form.time
									}
								...
								inputdata.settings = {
									'datacenter' : data.datacenter,
									'floorlist' : data.floor,
									'cabinet' : $scope.choices,
									'update_interval' : {
										'intervalType' : data.updateInterval,
										'intervalTime' : $scope.form.time
									}
								...
								inputdata.settings = {
									'formula' : $scope.choices
								};
								inputdata.settings.days = data.days;
								inputdata.settings.dateRangeFrom = $filter(
										'DateDividerAndFormatterForDash')(
										data.dateRange)[0];
								inputdata.settings.dateRangeTo = $filter(
										'DateDividerAndFormatterForDash')(
										data.dateRange)[1];
								inputdata.settings.datatype = data.datatype;
								inputdata.settings.subdatatype = data.subdatatype;
								inputdata.settings.update_interval = {
									'intervalType' : data.updateInterval,
									'intervalTime' : $scope.form.time
								}
								inputdata.dashboardId = $rootScope.selectedDashboardIdForAddWidget;
							...
							} else if (widget.jsfunctionname === "area_sensor"
									|| widget.jsfunctionname === "bar_sensor"
									|| widget.jsfunctionname === "line_sensor") {
								inputdata = $scope.setSaveSettings();
								...
								_.filter(inputdata.settings.formula, function(
										choice, i) {
							...
							Notification.error({
								message : Messages.PLEASE_FILL_TITLE_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.widgetName && form.widgetName.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_SUB_TITLE_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.formula && form.formula.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_FORMULA_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.sensortype && form.sensortype.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_SENSOR_TYPE_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.metrixtype && form.metrixtype.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_TEMPERATURE_TYPE_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.thresholdType
								&& form.thresholdType.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_THRESHOLD_TYPE_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.feed && form.feed.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_FEED_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.phase && form.phase.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_PHASE_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.metrix && form.metrix.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_METRIC_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.datatype && form.datatype.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_DATA_TYPE_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.subdatatype
								&& form.subdatatype.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_SUB_DATA_TYPE_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.datacenter && form.datacenter.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_DATACENTER_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.floor && form.floor.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_FLOOR_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.cabinet && form.cabinet.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_CABINET_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.sensor && form.sensor.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_SENSOR_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.powerMetriceItem
								&& form.powerMetriceItem.$invalid) {
							Notification.error({
								message : Messages.PLEASE_FILL_Item_FIELD,
								delay : Constants.DELAY
							});
						} else if (form.daterange && form.daterange.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_DATE_RANGE_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.UpdateIntervalArray
								&& form.UpdateIntervalArray.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_UPDATE_INTERVAL_FIELD,
										delay : Constants.DELAY
									});
						} else if (form.time && form.time.$invalid) {
							Notification
									.error({
										message : Messages.PLEASE_FILL_INPUTS_FOR_TIME_FIELD,
										delay : Constants.DELAY
									});
						} else {
							Notification
									.error({
										message : Messages.PLEASE_FILL_MANDATORY_FIELDS,
										delay : Constants.DELAY
									});
						}
					}
